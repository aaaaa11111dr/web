...
<?php if ($isResult): ?>
<header class="result-topbar"><a class="result-logo" href="/"><?= brand_text($settings['site_name']) ?></a><form class="result-search-box" method="get" action="/"><input name="q" type="search" value="<?= e($q) ?>" maxlength="160" autofocus><button type="submit" aria-label="搜索"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></button></form><nav class="result-nav"></nav></header>
...
<?php if ($data): ?>
  ...
  <?php else: ?>
    ...
    $engineParam = '';
    if ($data['page'] > 1): ?><a class="button secondary pager-link" href="/?q=<?= urlencode($q) ?>&page=<?= $data['page'] - 1 ?><?= $engineParam ?>">‹</a><?php endif;
    for ($p = $start; $p <= $end; $p++):
      if ($p === $data['page']): ?><span class="page-current"><?= $p ?></span><?php else: ?><a class="button secondary pager-link" href="/?q=<?= urlencode($q) ?>&page=<?= $p ?><?= $engineParam ?>"><?= $p ?></a><?php endif;
    endfor;
    if ($data['page'] < $maxPage): ?><a class="button secondary pager-link" href="/?q=<?= urlencode($q) ?>&page=<?= $data['page'] + 1 ?><?= $engineParam ?>">›</a><?php endif; ?></nav>
