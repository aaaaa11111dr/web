...
$stmt = pdo()->prepare('SELECT * FROM `search_logs` ORDER BY `id` DESC LIMIT ' . (int)$logsPerPage . ' OFFSET ' . (int)$logsOffset);
$stmt->execute();
$logs = $stmt->fetchAll();
$proxyStmt = pdo()->prepare('SELECT * FROM `page_proxy_logs` ORDER BY `id` DESC LIMIT ' . (int)$proxyLogsPerPage . ' OFFSET ' . (int)$proxyLogsOffset);
$proxyStmt->execute();
